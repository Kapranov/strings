...
# algorithm that can be cracked before humanity is wiped out should be enough for
# people to not use it for privacy related data.
...
# the actual root of the problem. Time for an example. Assuming we have a hashing
# function called "hash" and two strings, A and B (where A and B are unique), our
# hashing process of these strings would look like the following:
...
# period of time before they're able to crack the password, this is a very stupid
# mistake. While trying to crack a password (by bruteforcing it for example) may
# take a long time on a single computer most hackers can easily boot up a few
# servers or even worse, use a botnet. All known hashing algorithms (except
# BCrypt, more on that later) are affected by a single common problem: Moore's
# law. Moore's law states that every two years the amount of transistors that can
# be put in a computer doubles. This means that the faster computers get the
# quicker they're able to crack a password. A hacker merely has to use N
# computers and the time required to retrieve the original password will be
# greatly reduced.
...
# In order to explain this properly I simplified the process of hashing A N
# times:
...
# called "BCrypt". BCrypt is a hashing algorithm based on Blowfish with a small
# twist: it keeps up with Moore's law. The idea of BCrypt is quite simple, don't
# just use regular characters (and thus increasing the entropy) and make sure
# password X always takes the same amount of time regardless of how powerful the
# hardware is that's used to generate X. I'm not going to cover all the technical
# details but basically BCrypt requires you to specify a cost/workfactor in order
# to generate a password. This workfactor not only makes the entire process
# slower but is also used to generate the end hash. This means that if somebody
# were to change the workfactor the hash would also be different. In other words,
# hackers, you're fucked. In order for a hacker to gain the original password he
# must use the same workfactor and thus has to wait N times longer than when not
# using a workfactor.
