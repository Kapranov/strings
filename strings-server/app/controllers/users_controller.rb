class UsersController < AuthenticationController
  ...
  def create
    ...
    if @user.save
      render json: Oj.dump(json_for(@user, include: ['phones', 'cards'], meta: meta), mode: :compat), status: :created, message: "User was created"
    else
      return head :unauthorized
    end
  end
  ...
  def update
    if @user.update(user_params)
      render json: Oj.dump(json_for(@user, include: ['phones', 'cards'], meta: meta), mode: :compat), status: :ok, message: "User was updated"
    else
      return head :unauthorized
    end
    #unless @current_user.admin == true
